...
var FSHADER_SOURCE = `
    ...
    void main(){
        ...
        }else if( distance( texColor, vec3(1.0, 0.0, 0.0) ) < 0.3){
          //  red cloth and hat
          vec3 colorFromHeartTexture = texture2D( u_Sampler2, v_TexCoord ).rgb;
          if(distance( colorFromHeartTexture,  vec3(1.0, 1.0, 1.0) ) < 0.7){
            vec3 colorFromMarioD = texture2D( u_Sampler0, v_TexCoord ).rgb;
            texColor = colorFromMarioD * 0.4 + colorFromHeartTexture * 0.6;
          }
        }
...
function draw(){
    ...
    //"marioD.jpg","LVTexture.jpg","heart.jpg"
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, textures["marioD.jpg"]);
